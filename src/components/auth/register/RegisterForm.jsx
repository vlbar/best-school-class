...
const registerSchema = yup.object().shape({
    email: yup.string().trim().email("Неверный email").required("Вы не ввели email!"),
    secondName: yup
        .string()
        .trim()
...
function RegisterForm() {
    ...
    const submit = (values, { setSubmitting }) => {
        ...
        register(values)
            ...
            .catch(e => {
                if (e.response.status === 409) setErrorMessage("Email уже используется!");
        ...
        <Container className="register-form">
            ...
            <div className="form-wrapper">
                ...
                <Formik
                    ...
                    {({ dirty, isValid, touched, isSubmitting, submitForm, setFieldError, errors }) => (
                        <Form
                            onSubmit={e => {
                                e.preventDefault();
                            ...
                            <Row>
                                <Col md={6}>
                                    <FastField
                                        className={
                                            "form-control input " +
                                            (errors.firstName && touched.firstName ? "border-danger" : touched.firstName ? "border-success" : "")
                                        }
                                        name="firstName"
                                        type="text"
                                ...
                                <Col md={6}>
                                    <FastField
                                        className={
                                            "form-control input " +
                                            (errors.secondName && touched.secondName ? "border-danger" : touched.secondName ? "border-success" : "")
                                        }
                                        name="secondName"
                                        type="text"
                            ...
                            <FastField
                                className={
                                    "form-control input " +
                                    (errors.middleName && touched.middleName ? "border-danger" : touched.middleName ? "border-success" : "")
                                }
                                name="middleName"
                                type="text"
                                    ...
                                    registerSchema
                                        ...
                                        .then(async () => {
                                            emailUniqueTest.current(email).then(result => {
                                                setTimeout(() => setFieldError("email", result ? undefined : "Email занят!"), 0);
                                            });
                                        })
                                        .catch(() => {});
                                }}
                            ...
                            <FastField
                                className={
                                    "form-control input " +
                                    (errors.password && touched.password ? "border-danger" : touched.password ? "border-success" : "")
                                }
                                name="password"
                                type="password"
                            ...
                            />
                            <Form.Group>
                                <Button variant="primary" className="btn-block" type="submit" disabled={!(dirty && isValid && status === "success")}>
                                    <div className="w-100 d-flex justify-content-center">
                                        {isSubmitting ? <Spinner radius={21} color="#ECF0F6" stroke={2} /> : "Зарегистрироваться"}
                                    </div>
                                </Button>
                            </Form.Group>
                        </Form>
